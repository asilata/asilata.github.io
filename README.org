...
The github container's org version is **old**, so we have had to make some changes.
I will eventually run this in a docker emacs container with a newer version of org, as described [[https://duncan.codes/posts/2019-09-03-migrating-from-jekyll-to-org/][here]].
...
For the moment we are not using ~org-collect-keywords~, until I can run a newish version of org on the github builder.
#+begin_src emacs-lisp :tangle "build.el"
  (defun make-navbar ()
...
#+begin_src yaml :tangle ".github/workflows/build.yml" :mkdirp yes
  ...
  jobs:
    build:
      ...
      - name: Install emacs
        run: sudo add-apt-repository ppa:kelleyk/emacs --yes
